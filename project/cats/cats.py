...
def about(subject):
    ...
    def check(paragraph):
        ...
        para_words = split(paragraph)
        for i in range(len(subject)):
            if subject[i] in para_words:
                  for s in range(len(para_words)):
                      if para_words[s] == subject[i]:
                          return True
        return False              
    return check                
...
def accuracy(typed, source):
    ...
    if (typed_words == [] and source_words != [])or (typed_words != [] and source_words == []):
        return 0.0
    elif source_words == []and typed_words == []:
        return 100.0
    else:
        num_words = max(len(typed_words),len(source_words))
        right_words =0
        for i in range(min(len(typed_words),len(source_words))):
            if typed_words[i] == source_words[i]:
                right_words +=1
        return right_words/num_words*100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in word_list:
        if typed_word == i:
            return typed_word
    dif = 0
    simmilar = word_list[0]
    dif_min = diff_function(typed_word,word_list[0],limit)
    for i in word_list:
       dif = diff_function(typed_word,i,limit)
       if dif<dif_min:
           dif_min = dif
           simmilar = i
    if dif_min > limit:
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit == -1:
        ...
        if typed[0]==source[0]:
            ...
            substitute = 1 + int(minimum_mewtations(source[0] + typed[1:], source, limit - 1))
...
def report_progress(typed, source, user_id, upload):
    ...
    """
    # BEGIN PROBLEM 8
    def count_progress(typed,source):
        for i in range(len(typed)):
            if typed[i] != source[i]:
                return i/len(source)
        return len(typed)/len(source)
            
    progress = count_progress(typed,source)   
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        ...
        for j in range(len(tpp[i])-1):
            list_new.append(tpp[i][j+1]-tpp[i][j])
        
        times.append(list_new)
...
def fastest_words(words_and_times):
    ...
    for j in word_indices:
        ...
        for k in player_indices:
            if get_time(times,k,j) < min_time:
                min_time = get_time(times,k,j)
                min_player = k
        ans_list[min_player].append(words[j])
    return ans_list
    # END PROBLEM 10
