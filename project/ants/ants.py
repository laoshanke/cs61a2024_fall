...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if self.double_test == False:
            self.damage *= 2
            self.double_test = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def distance_front(place1,place2):
            if place1 == place2:
                return 0
            else:
                return distance_front(place1.entrance,place2) + 1

        def nearest_bee_place(place):
            
            if place.is_hive:
                return None
            elif place.bees and distance_front(self.place,place) <= self.upper_bound and distance_front(self.place,place) >= self.lower_bound:
                return place
            else:
                return nearest_bee_place(place.entrance)
        place_nearest_bee = nearest_bee_place(self.place)
        if place_nearest_bee :
            return random_bee(nearest_bee_place(self.place).bees)
        else:
            return None
        # END Problem 3 and 4
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if other.is_container is True:
            return False
        elif self.ant_contained is None:
            return True
        else:
            return False
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        #三种情况 没有/容器/非容
        while tmp_place:
            if isinstance(tmp_place.ant, ContainerAnt):
                tmp_place.ant.double()
                if tmp_place.ant.ant_contained:
                    tmp_place.ant.ant_contained.double()
            elif tmp_place.ant:
                tmp_place.ant.double()
            tmp_place = tmp_place.exit
        # END Problem 12
